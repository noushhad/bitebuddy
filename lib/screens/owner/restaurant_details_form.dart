...
class _RestaurantDetailsFormState extends State<RestaurantDetailsForm> {
  ...
  Future<void> _loadRestaurant() async {
    ...
    if (restaurant != null) {
      ...
      _descriptionController.text = restaurant['description'] ?? '';
      _selectedCuisines = List<String>.from(restaurant['tags'] ?? []);
      _priceLevel = restaurant['price_level'] as int?; // <--- ADDED
              ...
              _SectionCard(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 12),
                    // --- Price Level Dropdown Added Here ---
                    // --- Price Level Dropdown Fixed with Taka Symbol ---
                    DropdownButtonFormField<int>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                            value: 1, child: Text('৳ (Inexpensive)')),
                        DropdownMenuItem(
                            value: 2, child: Text('৳৳ (Moderate)')),
                        DropdownMenuItem(
                            value: 3, child: Text('৳৳৳ (Expensive)')),
                        DropdownMenuItem(
                            value: 4, child: Text('৳৳৳৳ (Very Expensive)')),
                      ],
                      onChanged: (val) => setState(() => _priceLevel = val),
                      validator: (val) =>
                          val == null ? 'Choose price range' : null,
                    ),
