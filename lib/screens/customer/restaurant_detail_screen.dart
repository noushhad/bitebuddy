...
import 'package:bitebuddy/screens/common/map_screen.dart';
import 'package:google_maps_flutter/google_maps_flutter.dart';
import 'package:bitebuddy/services/foodpanda_menu_service.dart';
import 'package:bitebuddy/services/menu_image_service.dart';
...
class _RestaurantDetailScreenState extends State<RestaurantDetailScreen> {
  ...
  // ✅ Services
  late final FoodpandaMenuService _foodpandaService = FoodpandaMenuService();
  late final MenuImageService _menuImageService = MenuImageService();
  ...
  Future<void> _loadRandomMenuImagesFromSupabase() async {
    try {
      final randomImages = await _menuImageService.getRandomMenuImages(1);

      if (randomImages.isNotEmpty) {
        setState(() {
          _placePhotoUrls = randomImages;
        });
        debugPrint('✅ Loaded ${randomImages.length} random menu images');
      } else {
        debugPrint('⚠️ No menu images found in Supabase');
      }
    } catch (e) {
      debugPrint('❌ Error loading random images: $e');
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // Show menu images from Supabase
          if (_placePhotoUrls.isNotEmpty) ...[
            const Text('Menu & Images',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
